...
const EstateSelectColumn = "id ,name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity, popularity_negative"
...
type Chair struct {
	...
	PopularityNegative int64  `db:"popularity_negative" json:"-"`
...
func postChair(c echo.Context) error {
	...
	const BULK_SIZE = 100
	var chairs []Chair = make([]Chair, 0, BULK_SIZE)
...
func searchChairs(c echo.Context) error {
	...
	limitOffset := " ORDER BY popularity_negative ASC, id ASC LIMIT ? OFFSET ?"
...
func getEstateDetail(c echo.Context) error {
	...
	var estate Estate
	err = db.Get(&estate, fmt.Sprintf("SELECT %s FROM estate WHERE id = ?", EstateSelectColumn), id)
	if err != nil {
		if err == sql.ErrNoRows {
...
func postEstate(c echo.Context) error {
	...
	const BULK_SIZE = 100
	var estates []Estate = make([]Estate, 0, BULK_SIZE)
	for _, row := range records {
		rm := RecordMapper{Record: row}
		estate := Estate{
			ID:          int64(rm.NextInt()),
...
func searchEstates(c echo.Context) error {
	...
	searchQuery := fmt.Sprintf("SELECT %s FROM estate WHERE ", EstateSelectColumn)
...
func getLowPricedEstate(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT %s FROM estate ORDER BY rent ASC, id ASC LIMIT ?`, EstateSelectColumn)
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	h := chair.Height
	d := chair.Depth
	query = fmt.Sprintf(`SELECT %s FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) ORDER BY popularity_negative ASC, id ASC LIMIT ?`, EstateSelectColumn)
...
func searchEstateNazotte(c echo.Context) error {
	...
	estates := []Estate{}
	query := fmt.Sprintf(`SELECT %s FROM estate WHERE ST_Contains(ST_PolygonFromText(%s), point) ORDER BY popularity_negative, id LIMIT ?`, EstateSelectColumn, coordinates.coordinatesToText())
...
func postEstateRequestDocument(c echo.Context) error {
	...
	estate := Estate{}
	query := fmt.Sprintf(`SELECT %s FROM estate WHERE id = ?`, EstateSelectColumn)
